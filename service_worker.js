...
const EXTENSIONS_TO_CACHE = ['.js', '.css', '.png', '.jpg', '.jpeg', '.gif', '.mp4'];

// Define files to exclude from cache
const EXCLUDED_FILES = [
  '/wp-content/themes/wp-twig/static/scripts/site.js',
];
...
function shouldCache(url) {
  const isExcluded = EXCLUDED_FILES.some(excludedUrl => url.includes(excludedUrl));
  if (isExcluded) {
    return false;
  }
...
self.addEventListener('fetch', event => {
  ...
  if (requestUrl.origin === location.origin && shouldCache(requestUrl.pathname)) {
    event.respondWith(
